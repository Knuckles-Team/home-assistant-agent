...
KG ingestion is authoritative: engine, validation, and transaction failures from
the `home_ingest_*` tools are surfaced explicitly.
...
- Engine, validation, and transaction failures propagate; an ingest result is only
  returned after the authoritative graph write succeeds.
